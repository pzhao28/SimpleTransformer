...
class Generator(nn.Module):
  ...
  def __init__(self, d_model, vocab):
    super().__init__() # in python2 we use: super(Generator, self).__init__()
...
class Encoder(nn.Module):
  ...
  def __init__(self, layer, N):
    ...
    self.norm = nn.LayerNorm(layer.size) #归一化
...
class DecoderLayer(nn.Module):
  #Decoder is made of self-attn, src-attn, and feed forward (defined bellow)
...
class MultiHeadedAttention(nn.Module):
  ...
  def forward(self, query, key, value, mask=None):
    ...
    # 3) "Contact" using a view and apply a final linear.
    x = x.transpose(1,2).contiguous().view(nbatches, -1, self.h * self.d_k) #x.transpose(1,2) ???
    return self.linears[-1](x)


class PositionwiseFeedForward(nn.Module): #Called by EncoderLayer() and DecoderLayer()
  #Implements FFN equation
  def __init__(self,d_model,d_ff,dropout=0.1):
    super().__init__()
    self.w_1 = nn.Linear(d_model, d_ff) #d_model: dimensionality of input and output; d_ff: dimensionality of inner layer
...
def batch_size_fn(new, count, sofar):
  ...
  return max(src_elements, tgt_element)
